...
class TextAnnotationObj extends annotation_types_1.MarkupAnnotationObj {
    ...
    createDefaultAppearanceStream() {
        ...
        if (this.opacity !== 1) {
            ...
            this.additional_objects_to_write.push({
                obj: gsp,
                func: (ob) => ob.writeGStateParameter(),
            });
        ...
        switch (this.icon) {
            ...
            case AnnotationIcon.Paragraph:
                ...
                break;
            default:
                go.drawPolygon([10, 10, 10, 10]).drawLine(80, 90, 90, 80);
        }
        this.appearanceStream.N = xobj;
        this.additional_objects_to_write.push({
            obj: xobj,
            func: (ob, cryptoInterface) => ob.writeXObject(cryptoInterface),
        });
    }
    ...
    validate(enact = true) {
        ...
        if (this.state && this.stateModel) {
            if (this.stateModel.valueOf() === AnnotationStateModel.Marked) {
                if (this.state.valueOf() !== AnnotationState.Marked &&
                ...
                if (this.state.valueOf() !== AnnotationState.Accepted &&
